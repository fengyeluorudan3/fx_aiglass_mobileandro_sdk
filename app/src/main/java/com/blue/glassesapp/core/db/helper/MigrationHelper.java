...
public class MigrationHelper {
    ...
    private static void reflectMethod(Database db, String methodName, boolean isExists, @NonNull Class<? extends AbstractDao<?, ?>>... daoClasses) {
        ...
        try {
            ...
            LogUtils.e(TAG, "【Failed to invoke " + methodName + " from " + e.getMessage());
        }
    }
     ...
     * 是否强制设置默认值
     */
    private static final boolean isForceSetDefaultValue = true;
        ...
        for (int i = 0; i < daoClasses.length; i++) {
            ...
            try {
                ...
                for (TableInfo tableInfo : newTableInfos) {
                    if (isForceSetDefaultValue || tableInfo.notnull && !tempTableInfos.contains(tableInfo)) {
                        String column = '`' + tableInfo.name + '`';
                        intoColumns.add(column);

                        String value;
                        if (tableInfo.dfltValue != null && !tableInfo.dfltValue.isEmpty()) {
                            value = "'" + tableInfo.dfltValue + "' AS ";
                        } else {
                            value = getDefaultValue(tableInfo.type) + " AS ";
                        }
                ...
                if (intoColumns.size() != 0) {
                    String insertTableStringBuilder = "REPLACE INTO " + tableName + " (" + TextUtils.join(",", intoColumns) + ") SELECT " + TextUtils.join(",", selectColumns) + " FROM " + tempTableName + ";";
                    db.execSQL(insertTableStringBuilder);
    ...
    private static String getDefaultValue(String type) {
        if (type.contains("CHAR") || type.contains("TEXT") || type.contains("CLOB")) {
            return "''";
        } else if (type.contains("INT") || type.contains("LONG") || type.contains("DOUBLE") || type.contains("FLOAT")) {
            return "0";
    ...
    private static class TableInfo {
        ...
        public String toString() {
            return "TableInfo{" + "cid=" + cid + ", name='" + name + '\'' + ", type='" + type + '\'' + ", notnull=" + notnull + ", dfltValue='" + dfltValue + '\'' + ", pk=" + pk + '}';
        }
